...
export const metadata = { title: "Travel readiness · Pawdex" };
export const dynamic = "force-dynamic";

const DEFAULT_DESTINATION: Destination =
  ALL_DESTINATIONS.find((d) => d.code === "FR") ?? ALL_DESTINATIONS[0];
...
export default async function EuTravelPage({
  ...
  const destination =
    ALL_DESTINATIONS.find((d) => d.code === sp.to) ?? DEFAULT_DESTINATION;
  const travelDate =
  ...
  const [petRes, vaccRes, medsRes, eventsRes] = await Promise.all([
    supabase
      .from("pets")
      .select(
        "name, species, date_of_birth, microchip_number, microchip_registry, microchip_implanted_on",
      )
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            <div>
              ...
              >
                {destination.regime === "gb"
                  ? "Compliance against GB pet travel rules (gov.uk)"
                  : "Compliance against EU rules applying from 22 Apr 2026"}
        ...
        >
          ...
          substitute for a USDA-accredited vet&apos;s sign-off on the
          destination&apos;s health certificate (the EU and GB each have their
          own, issued via VEHCS and endorsed by USDA APHIS — neither is APHIS
          Form 7001). The rules above reflect the destination regime as Pawdex
          understands it (EU: Reg (EU) 2026/131 and 2026/636 applying from 22
          April 2026; GB: gov.uk guidance) — verify with your vet and the
          destination&apos;s competent authority before travel.
        </footer>
      </div>
    </>
  );
...
function OverallBadge({
  ...
  const map = {
    ready: {
      // "Records ready", not "Ready to travel": the health certificate can
      // only exist inside the 10-day pre-travel window, so record-readiness is
      // the strongest true claim this page can make.
      label: "Records ready — book the certificate",
      bg: "var(--pw-accent-soft)",
      fg: "var(--pw-accent-fg-on-soft)",
      icon: "checkCircle",
    },
